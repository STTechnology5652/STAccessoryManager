...
class STCameraVC: STABaseVC {
    ...
    private var streamPlayer: STH264StreamPlayer?
    ...
    override func viewDidLayoutSubviews() {
        ...
        videoLayer?.frame = vDisplayContainer.bounds
    }
}
    ...
    func bindVMOutput(_ output: STCameraVM.STCameraOutput) {
        ...
        output.h264Data
            .drive(onNext: { [weak self] data in
                self?.streamPlayer?.processH264Data(data)
            })
            .disposed(by: disposeBag)
        ...
        output.deviceState
            .drive(onNext: { [weak self] state in
                ...
                switch state {
                case .connected:
                    self.imgMedi.isHidden = true
                    self.videoLayer?.isHidden = false
                case .disconnected:
                    self.imgMedi.isHidden = false
...
private extension STCameraVC {
    ...
    func setupDisplayContainer() {
        ...
        do {
            streamPlayer = try STH264StreamPlayer()
            if let displayLayer = streamPlayer?.getDisplayLayer() {
                displayLayer.frame = vDisplayContainer.bounds
                displayLayer.videoGravity = .resizeAspectFill
                vDisplayContainer.layer.addSublayer(displayLayer)
                videoLayer = displayLayer
                
                // 添加调试信息
                STLog.debug("Video layer frame: \(displayLayer.frame)")
                STLog.debug("Display container frame: \(vDisplayContainer.frame)")
            }
        } catch {
            STLog.err("Failed to create stream player: \(error)")
        }
